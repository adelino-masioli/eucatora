...
@section('body')
    @include('providers::partial.menu')
    <div class="row">
        <div class="col-lg-12">
            <table class="table table-bordered table-striped table-responsive table-condensed table-hover datatable" id="providers-table">
                <thead>
                <tr>
    ...
    <div id="urlaction" style="display: none;">{{url('dashboard/provider/destroy')}}</div>
...
<script>
    var table = $('#providers-table').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: '{!! url('dashboard/provider/datatable') !!}',
            data: function (d) {
...
{{HelperJS::autocomplete(url('dashboard/provider/autocomplete?query=%QUERY'), '#autoname', 3)}}
@endpush